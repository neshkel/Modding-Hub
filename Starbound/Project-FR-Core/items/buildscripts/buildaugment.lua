...
--- Core build script for augmentation modules (EPP Augments, Pet Collars).
--- Handles dynamic translation of effect names, descriptions,
--- and metadata injection (displayName) into item parameters.
function build(directory, config, parameters, level, seed)
  ...
  if generator and generator.generateTooltips then
    local status, err = pcall(function()
      ...
      generator.generateTooltips(config, parameters, level, seed, "augments")
    end)
    
    if not status then
      -- Log critical failures while maintaining item integrity.
      sb.logError("AUGMENT GENERATION ERROR: %s", err)
    end
  else
    -- Debug notice: Generator scope usually loads after initial recipe scanning.
    sb.logDebug("Generator scope not ready for augment: %s", config.itemName or "unknown")
  end
  -- ==========================================================
