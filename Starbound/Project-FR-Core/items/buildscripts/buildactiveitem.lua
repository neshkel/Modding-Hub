...
--- Core build script for multi-purpose active items.
--- Orchestrates script-driven behavior mapping, animation metadata,
...
function build(directory, config, parameters, level, seed)
  ...
  if generator and generator.generateTooltips then
    ...
    if not status then
      ...
      sb.logError("ACTIVEITEM GENERATION ERROR: %s", err)
    end
  else
    -- Debug notice: Generator scope usually loads after initial recipe scanning.
    sb.logDebug("Generator scope not ready for activeitem: %s", config.itemName or "unknown")
  end
  -- ==========================================================
