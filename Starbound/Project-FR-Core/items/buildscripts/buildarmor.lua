...
--- Core build script for protective gear and vanity armors sets.
--- Handles dynamic translation of set names, piece descriptions,
...
function build(directory, config, parameters, level, seed)
  ...
  if generator and generator.generateTooltips then
    local status, err = pcall(function()
      ...
      generator.generateTooltips(config, parameters, level, seed, "armors")
    end)
    
    if not status then
      -- Log critical failures while maintaining item integrity.
      sb.logError("ARMOR GENERATION ERROR: %s", err)
    end
  else
    -- Debug notice: Generator scope usually loads after initial recipe scanning.
    sb.logDebug("Generator scope not ready for armor: %s", config.itemName or "unknown")
  end
  -- ==========================================================
