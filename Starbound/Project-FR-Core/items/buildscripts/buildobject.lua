...
--- Core build script for static and interactive objects.
--- Handles atmospheric description mapping, interaction metadata,
--- and secure asset localization through the shared engine.
function build(directory, config, parameters, level, seed)
  ...
  if generator and generator.generateTooltips then
    local status, err = pcall(function()
      ...
      generator.generateTooltips(config, parameters, level, seed, "objects")
    end)
    
    if not status then
      -- Log critical failures while maintaining item integrity.
      sb.logError("OBJECT GENERATION ERROR: %s", err)
    end
  else
    -- Debug notice: Generator scope usually loads after initial recipe scanning.
    sb.logDebug("Generator scope not ready for object: %s", config.objectName or "unknown")
  end
  -- ==========================================================
