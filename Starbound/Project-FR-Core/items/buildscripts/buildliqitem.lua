...
--- Core build script for liquid resources and fluid materials.
--- Handles dynamic translation of substance names, hazard warnings,
--- and metadata injection (viscosity, status effects) into fluid parameters.
function build(directory, config, parameters, level, seed)
  ...
  if generator and generator.generateTooltips then
    local status, err = pcall(function()
      ...
      generator.generateTooltips(config, parameters, level, seed, "liqitems")
    end)
    
    if not status then
      -- Log critical failures while maintaining item integrity.
      sb.logError("LIQUID GENERATION ERROR: %s", err)
    end
  else
    -- Debug notice: Generator scope usually loads after initial recipe scanning.
    sb.logDebug("Generator scope not ready for liquid: %s", config.itemName or "unknown")
  end
  -- ==========================================================
