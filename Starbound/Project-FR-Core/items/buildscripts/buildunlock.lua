...
--- Core build script for progression unlockables and ship licenses.
--- Handles tier-based metadata validation, ship expansion protocols, 
--- and secure translation of expansion-related labels and effects.
function build(directory, config, parameters, level, seed)
  ...
  if generator and generator.generateTooltips then
    local status, err = pcall(function()
      ...
      generator.generateTooltips(config, parameters, level, seed, "unlocks")
    end)
    
    if not status then
      -- Log critical failures while maintaining item integrity.
      sb.logError("UNLOCK GENERATION ERROR: %s", err)
    end
  else
    -- Debug notice: Generator scope usually loads after initial recipe scanning.
    sb.logDebug("Generator scope not ready for unlock: %s", config.itemName or "unknown")
  end
  -- ==========================================================
