...
--- Core build script for basic inventory items and consumables.
--- Handles inventory metadata mapping, stacking property localization,
--- and secure asset generation through the shared engine.
function build(directory, config, parameters, level, seed)
  ...
  if generator and generator.generateTooltips then
    ...
    if not status then
      ...
      sb.logError("ITEM GENERATION ERROR: %s", err)
    end
  else
    -- Debug notice: Generator scope usually loads after initial recipe scanning.
    sb.logDebug("Generator scope not ready for item: %s", config.itemName or "unknown")
  end
  -- ==========================================================
