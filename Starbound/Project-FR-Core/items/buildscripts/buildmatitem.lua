...
--- Core build script for material-type items and placeable blocks.
--- Orchestrates material index mapping, collision metadata, 
--- and secure asset localization through the shared generator engine.
function build(directory, config, parameters, level, seed)
  ...
  if generator and generator.generateTooltips then
    local status, err = pcall(function()
      ...
      generator.generateTooltips(config, parameters, level, seed, "matitems")
    end)
    
    if not status then
      -- Log critical failures while maintaining item integrity.
      sb.logError("MATITEM GENERATION ERROR: %s", err)
    end
  else
    -- Debug notice: Generator scope usually loads after initial recipe scanning.
    sb.logDebug("Generator scope not ready for matitem: %s", config.itemName or "unknown")
  end
  -- ==========================================================
