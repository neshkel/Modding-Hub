...
function init()
  --- @description Data Retrieval: Loads the quest-specific dictionary for localization.
  local t = generator.getDictionary("quests")
  
  storage.complete = storage.complete or false
  self.compassUpdate = config.getParameter("compassUpdate", 0.5)

  --- @description Dynamic Description Mapping: 
  --- Replaces the default quest description with the localized version from the dictionary.
  --- This ensures the objective text ("Wake up and reboot the ship") is translated in real-time.
  self.descriptions = config.getParameter("descriptions")
  if t and t.story and t.story.bootship and t.story.bootship.descriptions then
    self.descriptions.wakeSail = t.story.bootship.descriptions.wakesail
  end
