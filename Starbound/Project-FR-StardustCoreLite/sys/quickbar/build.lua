...
do -- build the actual list
  ...
  for _, i in pairs(c.items) do -- dump in normal items
    if not i.condition or condition(table.unpack(i.condition)) then
      -- Project FR: Localize item label using technical ID as key
      -- Matches path in generics.config: /ui/mods/stardustCoreLite/quickBar/[id]
      i.label = _T("quickBar." .. _, i.label)
